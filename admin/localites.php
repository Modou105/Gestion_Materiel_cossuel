    ...
    <section class="content">
      ...
      <div class="card card-primary">
          <div class="card-header">
            <h3 class="card-title">LOCALITES</h3>
          </div>
              <!-- /.card-header -->
              <!-- form start -->
          <?php
            ...
            <div class="card-body">
              <div class="form-group">
                <label for="exampleInputNom">Nom de la localité</label>
                <?php
              if(isset($_GET['idloc'])){
                if($cnx!=false){
                    $req=$cnx->prepare("select* from localites where IdLocalite='".$_GET['idloc']."'");
                    $req->execute();
                    $infoloc=$req->fetch();
                ?>
                <input type="text" name="nomloc" class="form-control" id="exampleInputName" value="<?php echo $infoloc['NomLocalite'] ?>" required>
                <input type="hidden" name="idloc" class="form-control"  value="<?php echo $infoloc['IdLocalite'] ?>" required>
                <?php
				$cnx=NULL;
              }
              }else{
                ?>
                <input type="text" name="nomloc" class="form-control" id="exampleInputName" placeholder="Enter la localité" required>
                <?php
              }
            ?>
              </div>
              <div class="form-group">
                <?php
                  if(isset($_GET['idloc'])){
                    ?>
                      <input type="hidden" name="iddep" class="form-control" id="exampleInputName" required>
                    <?php
                  }else{
                    ?>
                      <input type="hidden" name="iddep" class="form-control" id="exampleInputName" value="<?php echo $_GET['iddep'] ?>" required>
                    <?php
                  }
                ?>
              </div>
          ...
          <div class="card-footer">
            <?php
              if(isset($_GET['idloc'])){
                ?>
                <button type="submit" name="modifLocalite" class="btn btn-primary">Modifier</button>
                <?php
              }else{
                ?>
                <button type="submit" name="enregLocalite" class="btn btn-primary">Enregistrer</button>
                <?php
              }
            ?>
          </div>
