    ...
    <section class="content">
      <div class="container-fluid">
        ...
        <div class="card-header">
            <h3 class="card-title">VISITES PLANIFIEES</h3>
          </div>
        <div class="row">
           <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    <?php
                    if(!isset($_GET['idag'])){
                      $reqVisitesPlanifiees=$connex->prepare("select dossiers.IdDossier, dossiers.NumeroDossier, dossiers.DateCreation, dossiers.HeureCreation, demandes.TypeDemande, demandes.NumeroDemande, demandes.DateDemande, affectations_dossiers.IdAgentAffectant from visites, dossiers, demandes, agents, affectations_agents, agences, affectations_dossiers where visites.IdDossier=dossiers.IdDossier and affectations_dossiers.IdDossier=dossiers.IdDossier and dossiers.IdDemande=demandes.IdDemande and affectations_dossiers.IdAgent=agents.IdAgent and agents.IdAgent=affectations_agents.IdAgent and affectations_agents.IdAgence=agences.IdAgence and agences.idAgence='".$_SESSION['idagence']."' and visites.EtatVisite='Planifie'");
                      }else{
                        $reqVisitesPlanifiees=$connex->prepare("select dossiers.IdDossier, dossiers.NumeroDossier, dossiers.DateCreation, dossiers.HeureCreation, demandes.TypeDemande, demandes.NumeroDemande, demandes.DateDemande, affectations_dossiers.IdAgentAffectant from visites, dossiers, demandes, agents, affectations_agents, agences, affectations_dossiers where visites.IdDossier=dossiers.IdDossier and affectations_dossiers.IdDossier=dossiers.IdDossier and dossiers.IdDemande=demandes.IdDemande and affectations_dossiers.IdAgent=agents.IdAgent and agents.IdAgent=affectations_agents.IdAgent and affectations_agents.IdAgence=agences.IdAgence and agences.idAgence='".$_SESSION['idagence']."' and agents.IdAgent='".$_GET['idag']."' and visites.EtatVisite='Planifie'");
                      }
                      $reqVisitesPlanifiees->execute();
                      $total=0;
                      while($infoVisitesPlanifiees=$reqVisitesPlanifiees->fetch()){
                    ?>
                  <tr>
                    <td><?php echo $infoVisitesPlanifiees['NumeroDossier'] ?></td>
                    <td><?php echo $infoVisitesPlanifiees['DateCreation'] ?></td>
                    <td><?php echo $infoVisitesPlanifiees['HeureCreation'] ?></td>
                    <td><?php echo $infoVisitesPlanifiees['NumeroDemande'] ?></td>
                    <td>
                      <?php 
                      if($infoVisitesPlanifiees['TypeDemande']=='dom'){
                      echo "Domestique"; 
                    }else{
                      echo "Non Domestique";
                    }
                    ...
                    <?php 
                    $rechAgent=$connex->prepare("select* from agents where IdAgent='".$infoVisitesPlanifiees['IdAgentAffectant']."'");
                    $rechAgent->execute();
