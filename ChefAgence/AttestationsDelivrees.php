    ...
    <section class="content">
      <div class="container-fluid">
        ...
        <div class="card-header">
            <h3 class="card-title">ATTESTATIONS DELIVREES</h3>
          </div>
        <div class="row">
           <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    <?php
                      $reqAttesDeliv=$connex->prepare("select attestations.NumeroAttestation, attestations.DateDelivrance, attestations.HeureDelivrance, dossiers.NumeroDossier, demandes.TypeDemande, demandes.NumeroDemande from attestations, dossiers, demandes, agents, affectations_agents, agences where attestations.IdDossier=dossiers.IdDossier and dossiers.IdDemande=demandes.IdDemande and demandes.IdAgent=agents.IdAgent and agents.IdAgent=affectations_agents.IdAgent and affectations_agents.IdAgence=agences.IdAgence and agences.idAgence='".$_SESSION['idagence']."'");
                      $reqAttesDeliv->execute();
                      $total=0;
                      while($infoAttesDeliv=$reqAttesDeliv->fetch()){
                    ?>
                  <tr>
                    <td><?php echo $infoAttesDeliv['NumeroAttestation'] ?></td>
                    <td><?php echo $infoAttesDeliv['DateDelivrance'] ?></td>
                    <td><?php echo $infoAttesDeliv['HeureDelivrance'] ?></td>
                    <td><?php echo $infoAttesDeliv['NumeroDossier'] ?></td>
                    <td><?php echo $infoAttesDeliv['NumeroDemande'] ?></td>
                    <td>
                      <?php 
                      if($infoAttesDeliv['TypeDemande']=='dom'){
                      echo "Domestique"; 
                    }else{
                      echo "Non Domestique";
                    }
                    ...
                    <tfoot>
                      <tr>
                        <td colspan="6"><strong><font size="5">TOTAL</font></strong></td>
