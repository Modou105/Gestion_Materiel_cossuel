      ...
      <div class="container-fluid">
        ...
        <div class="row">
           <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    <?php
                      $reqDossHorsLimite=$connex->prepare("select affectations_dossiers.IdAgentAffectant, dossiers.IdDossier, dossiers.NumeroDossier, dossiers.DateCreation, dossiers.HeureCreation,dossiers.DateCloture, demandes.TypeDemande, demandes.NumeroDemande, demandes.DateDemande, affectations_dossiers.IdAgentAffectant from affectations_dossiers, dossiers, demandes, agents, affectations_agents, agences where affectations_dossiers.IdDossier=dossiers.IdDossier and dossiers.IdDemande=demandes.IdDemande and demandes.IdAgent=agents.IdAgent and agents.IdAgent=affectations_agents.IdAgent and affectations_agents.IdAgence=agences.IdAgence and agences.idAgence='".$_SESSION['idagence']."' and dossiers.EtatDossier='Conforme' or dossiers.EtatDossier='Non Conforme' and DATEDIFF(dossiers.DateCreation, dossiers.DateCloture)>'5'");
                      $reqDossHorsLimite->execute();
                      $total=0;
                      while($infoDossHorsLimite=$reqDossHorsLimite->fetch()){
                    ?>
                  <tr>
                    <td><?php echo $infoDossHorsLimite['NumeroDossier'] ?></td>
                    <td><?php echo $infoDossHorsLimite['DateCreation'] ?></td>
                    <td><?php echo $infoDossHorsLimite['HeureCreation'] ?></td>
                    <td><?php echo $infoDossHorsLimite['NumeroDemande'] ?></td>
                    <td>
                      <?php 
                      if($infoDossHorsLimite['TypeDemande']=='dom'){
                      echo "Domestique"; 
                    }else{
                      echo "Non Domestique";
                    }
                    ...
                    <?php 
                    $rechAgent=$connex->prepare("select* from agents where IdAgent='".$infoDossHorsLimite['IdAgentAffectant']."'");
                    $rechAgent->execute();
                    ...
                    <?php
                    ?>
                    <td><?php echo $infoDossHorsLimite['DateCloture'] ?></td>
