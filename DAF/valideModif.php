    ...
    <section class="content">
      <div class="container-fluid">
        ...
        <?php
            if(isset($_POST['modifAgent'])){
              ...
              $idagent=$_POST['idagent'];
                  if(!empty($_FILES['monfichier']['name'])){
                    $req=$cnx->prepare("update agents set MatriculeAgent='$matag', PrenomAgent='$prenag', NomAgent='$nomag',AdresseAgent='$adrag',TelephoneAgent='$telag',EmailAgent='$mailag',PhotoAgent='$MaPhoto' where IdAgent='$idagent'");
                  }else{
                      $req=$cnx->prepare("update agents set MatriculeAgent='$matag', PrenomAgent='$prenag', NomAgent='$nomag',AdresseAgent='$adrag',TelephoneAgent='$telag',EmailAgent='$mailag' where IdAgent='$idagent'");
                  }
                  if($req->execute()){
                      ?>
                    ...
                    $cnx=NULL;
            }else if(isset($_POST['enregRemboursement'])){
              $iduser=$sess->RecuperationInformations('iduser');
              $motifrej=addslashes($_POST['motifrej']);
              $iddem=$_POST['iddem'];
                if($cnx!=false){
                    $req=$cnx->prepare("update paiements set EtatRemboursement='Rembourse' where IdDemande='$iddem'");
                    if($req->execute()){
                      $reqAnnule=$cnx->prepare("update demandes set EtatDemande='Remboursee' where IdDemande='$iddem'");
                      $reqAnnule->execute();
                      ?>
                        <script type="text/javascript">
            ...
            }else if(isset($_POST['enregRejet'])){
              $iduser=$sess->RecuperationInformations('iduser');
              $motifrej=addslashes($_POST['motifrej']);
              $iddem=$_POST['iddem'];
                if($cnx!=false){
                    $req=$cnx->prepare("update paiements set MotifRejet='$motifrej', EtatRemboursement='Rejete' where IdDemande='$iddem'");
                    if($req->execute()){
                      ?>
                        <script type="text/javascript">
                          alert('Demande de remboursement rejeté');
                          window.location.href="demandesARembourser.php";
                        </script>
                      <?php
                }else{
                  ?>
                        <script type="text/javascript">
                          alert('Erreur rejet demande de Remboursement');
                          window.location.href="demandesARembourser.php";
                        </script>
                      <?php
                }
                ...
                if($cnx!=false){
                    $req=$cnx->prepare("insert into compteutilisateurs(DateCreation, HeureCreation, login, Motdepasse, Etatcompte, Privilege, IdAgent) values('".$_POST['datecompte']."', '".$_POST['heurecompte']."','".addslashes($_POST['login'])."','".$_POST['pwd']."','Actif','comptable', '".$_POST['idag']."' )");
                    if($req->execute()){
                      ?>
                        <script type="text/javascript">
                          alert('Modification Effectuée');
