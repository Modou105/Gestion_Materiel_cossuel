	...
	class Utilisateurs{
		...
		function Verif_Utilisateur($con,$sess){
			...
			if($nouvelle_connexion!=false){
				$req_recherche=$nouvelle_connexion->prepare("select agent.Id_Agent, agent.Matricule, agent.Prenom_Agent, agent.Nom_Agent, agent.Telephone, agent.Email_Agent, compte_utilisateur.Statut , compte_utilisateur.Login from agent, compte_utilisateur where compte_utilisateur.Id_Agent=agent.Id_Agent and  compte_utilisateur.Login='".$this->login."' and compte_utilisateur.Password='".$this->motpasse."'");
				$req_recherche->execute();
				if($req_recherche->rowCount()>0){
					$resultat=$req_recherche->fetch();
					$sess->SauvegardeInformations('idagent',$resultat['Id_Agent']);
