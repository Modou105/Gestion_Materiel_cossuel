  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      <div class="container-fluid">
        ...
        <?php
             if(isset($_POST['enregDossier'])){
              $numdoss=$_POST['numdoss'];
              $datecree=$_POST['datecree'];
              $heurecree=$_POST['heurecree'];
              $iddem=$_POST['iddem'];
              $iduser=$sess->RecuperationInformations('iduser');
                if($connex!=false){
                    $req=$connex->prepare("insert into dossiers(NumeroDossier, DateCreation, HeureCreation, IdDemande,IdAgentCree,EtatDossier) values('$numdoss','$datecree','$heurecree','$iddem','$iduser','En Instance')");
                    if($req->execute()){
                      ?>
                ...
                if($connex!=false){
                    $req=$connex->prepare("insert into affectations_dossiers(DateAffectationDossier,HeureAffectationDossier, IdAgent,IdAgentAffectant, IdDossier) values('".date('Y-m-d')."', '".date('H:i:s')."','".$_POST['idagent']."','".$_POST['idagentAffectant']."','".$_POST['iddoss']."')");
                    if($req->execute()){
                      ?>
                        <script type="text/javascript">
                ...
                $cnx=NULL;
                 ...
                 ?>
                  ...
                  <?php   
                }else if(isset($_POST['clotureDossier'])){
                if($connex!=false){
                    $req=$connex->prepare("update dossiers set EtatDossier='".$_POST['infoclot']."' where IdDossier='".$_POST['iddoss']."'");
                    if($req->execute()){
                      ?>
                        <script type="text/javascript">
                          alert('Dossier Cloturé avec succès!');
                          window.location.href="VisitesTerminees.php";
                        </script>
                      <?php
                }else{
                  ?>
                        <script type="text/javascript">
                          alert('Erreur de cloture du dossier');
                          window.location.href="VisitesTerminees.php";
                        </script>
                      <?php
                    }
